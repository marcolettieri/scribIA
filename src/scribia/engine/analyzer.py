...
DETECTION_PATTERNS: dict[EntityType, list[re.Pattern[str]]] = {
    EntityType.CLASS: [
        ...
        re.compile(r"^\+\s*(?:public|private|protected|abstract|sealed)[\w\s]*class\s+(\w+)", re.MULTILINE),
    ...
    EntityType.API_ENDPOINT: [
        re.compile(r"""^\+.*@(?:app|router|blueprint)\.(?:get|post|put|patch|delete)\(['"]([\w/{}:]+)""", re.MULTILINE),
        re.compile(r"""^\+.*router\.(?:get|post|put|patch|delete)\(['"]([\w/{}:]+)""", re.MULTILINE),
        re.compile(r"""^\+.*@(?:Get|Post|Put|Patch|Delete|RequestMapping)\(['"]([\w/{}:]+)""", re.MULTILINE),
        re.compile(r"""^\+.*app\.(get|post|put|patch|delete)\(['"]([\w/{}:]+)""", re.MULTILINE),  # Express
    ],
    EntityType.DATA_MODEL: [
        re.compile(r"^\+\s*class\s+(\w+)\s*\([^)]*(?:Model|Schema|Entity|Base|BaseModel)[^)]*\)", re.MULTILINE),
        re.compile(r"^\+\s*@(?:dataclass|strawberry\.type|strawberry\.input)", re.MULTILINE),
        re.compile(r"^\+\s*(?:export\s+)?(?:interface|type)\s+(\w+)\s*(?:extends|=|{)", re.MULTILINE),
    ],
    EntityType.CONFIGURATION: [
        re.compile(r"^\+\s*(\w+)\s*=\s*os\.(?:getenv|environ\.get)", re.MULTILINE),
        re.compile(r"""^\+\s*(\w+)\s*:\s*(?:str|int|bool|float|Optional)\s*=\s*Field""", re.MULTILINE),
    ],
}
...
class SemanticAnalyzer:
        ...
        for entity_type, patterns in DETECTION_PATTERNS.items():
            for pattern in patterns:
                for match in pattern.finditer(fc.diff_content):
                    ...
                    name = (match.group(2) if match.lastindex and match.lastindex >= 2
    ...
    def _refine_type(
        ...
        if any(w in name_lower for w in ("service", "manager", "handler", "controller", "repository")):
